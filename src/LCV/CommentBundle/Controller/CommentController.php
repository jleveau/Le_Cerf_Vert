...
class CommentController extends Controller {
    ...
    public function viewPlaylistAction($playlist_id, Request $request) {
        $em = $this -> getDoctrine() -> getManager();
        $list_comment = $em -> getRepository('LCVCommentBundle:Comment') -> getPlaylistComments($playlist_id);
        $forms=array();
        $form_view=array();
    ...
    //       $this -> createForm(new EditCommentType(), $comment,array('id'=>$comment_id));
            ...
            if ($forms[$comment_id] -> handleRequest($request) -> isValid()) {
                ...
                return $this -> redirect($this -> generateUrl('lcv_playlist_view',array('playlist_id' => $playlist_id)));
            }
            $form_view[$comment_id]=$forms[$comment_id]->createView();
        }
        return $this -> render('LCVCommentBundle:Comment:show.html.twig', array('list_comment' => $list_comment,'forms'=>$form_view,'playlist_id'=>$playlist_id,'user_id'=>$user_id));
    }
